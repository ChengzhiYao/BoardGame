...
- verdict 只能是其一："是" / "不是" / "无关"（与真相无关或不影响）/ "是也不是"（部分正确或要看情况）。
- 绝不直接说出汤底；不要主动剧透。
- note 可留空；仅当玩家明显跑偏或非常接近时，给一句极短的中性提示（不超过15字），不要长篇。
- 如果玩家问的不是是非题（而是开放式问题），verdict 用 "无关"，note 提示"请用是非题提问"。

只输出 JSON：{ "verdict": "是/不是/无关/是也不是", "note": "可选的极短提示，没有就空字符串" }`;
}
