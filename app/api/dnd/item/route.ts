...
import { usePotion, buyPotion, buyGear, reviveAlly, equipWeapon } from '@/lib/dnd/engine';
...
export async function POST(req: Request) {
  ...
  const { roomId, action, kind, key, targetSeat, weaponIdx } = await req.json().catch(() => ({} as any));
  ...
  const out = await mutateState(admin, roomId, (s) => {
    if (action === 'equip') return equipWeapon(s, me.seat, Number(weaponIdx) || 0); // 持握武器是自由动作，战斗中也可
