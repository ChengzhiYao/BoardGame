...
【幕数·按本设计】根据这个剧本的体量自行决定**总幕数（5~8 幕）**：短小精悍的本 5 幕、信息量大/反转多的长本 7~8 幕。把每一幕写进 acts 数组（每幕 {name 幕名, goal 本幕目标}）。结构约定：${type === '推理' || type === '阵营' || type === '恐怖' ? '前面若干幕是开场/搜证/调查/对质等；**倒数第二幕必须是「最终指认」（投票指认/票决）**；**最后一幕是「真相揭晓」**。' : '前面若干幕推进剧情/情感/拼合/经营博弈，**没有指认环节**，**最后一幕是「结局揭晓 / 结算」**。'} name 是公开的幕名（不含剧透）。
...
export function buildJbsVotePrompt(caseFile: any, aiNames: string[], realVotes: { voter: string; target: string }[], discussion = '') {
  return `你是剧本杀 DM。隐藏档案（绝密）：
${JSON.stringify(caseFile).slice(0, 8000)}

【本局的实际讨论与指证过程（投票要据此判断，不能无视）】
${discussion || '（无明显讨论）'}

真人玩家的指认：${realVotes.map((v) => `${v.voter}→${v.target}`).join('；') || '（无）'}
AI 角色需各自独立投票：${aiNames.join('、') || '（无）'}。规则：
- 每个 AI 按**自己的角色逻辑与私利**投票（可能甩锅、护人、或真心指认），不得为了让真人获胜而故意改票；
- 但**必须结合上面真实发生的讨论**：如果某个玩家拿出了有力证据/逻辑、把怀疑成功引向某人、或打消了对自己的怀疑，AI 应当**据此调整判断**（被说服就改变看法，而不是无视讨论照投）。AI 不是无脑按剧本投，而是像真人一样会被证据和论证影响。
...
// 非指认型本（情感/还原/欢乐/机制）的结局揭晓 / 结算。
export function buildJbsEndingPrompt(caseFile: any, type: string, discussion = '') {
  const guide = type === '机制'
    ? '这是机制/经济本：依据隐藏资源表与本局博弈，结算每个角色的最终资源/分数，按机制值排名并宣布赢家。reveal 里讲清各人最终成绩、关键转折与谁赢了；scores 给每个角色一个 0~100 的机制值。'
    : type === '欢乐'
    ? '这是欢乐本：逐一揭晓每个人的搞笑秘密/任务完成情况，按"欢乐值"评 MVP/赢家。reveal 里抖包袱、公布乌龙真相；scores 给每个角色一个 0~100 的欢乐值。'
    : '这是情感/还原本：没有输赢。完整揭晓每个角色隐藏的秘密、目标、关系，以及"到底发生过什么"的全部往事，并给每个角色一个情感落点/结局。reveal 要饱满动人；winner 留空，scores 可留空。';
  return `你是剧本杀 DM，正在为本局做【结局揭晓 / 结算】（非指认型本）。隐藏档案（绝密）：
${JSON.stringify(caseFile).slice(0, 8000)}
...
${guide}
这是结案，可以全部公开。只输出 JSON：
{
  "reveal": "完整结局正文（可长：逐角色揭秘 + 总体真相/结算 + 各自结局/排名）",
  "scores": [ { "name": "角色名", "meter": 0 } ],
  "winner": "赢家角色名或阵营（情感/还原本留空）"
}`;
}
