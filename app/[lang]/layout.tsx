  ...
  return (
    <div className="min-h-screen text-parchment">
      <header className="max-w-5xl mx-auto flex items-center justify-between px-5 py-4">
        <Link href={`/${lang}`} className="font-serif text-xl">谜夜<span className="text-blood">.</span><span className="text-parchment/40 text-sm ml-1.5">MystNight</span></Link>
      ...
      {children}
      <footer className="max-w-5xl mx-auto px-5 py-10 mt-10 border-t border-eldritch/15 text-xs text-parchment/40">
        © {new Date().getFullYear()} MystNight 谜夜 · <Link href="/" className="hover:text-parchment">{en ? 'Enter the app' : '进入游戏'}</Link>
      </footer>
    </div>
  );
}
