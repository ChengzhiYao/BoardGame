...
export default function SoupRoom(props: ShellProps & { soupSurface?: string }) {
  ...
  async function reveal() { if (!confirm(en ? 'Reveal the answer? This ends the game.' : '确定要看汤底吗？看了这局就结束了。')) return; await call('/api/soup/reveal', { roomId: props.room.id }); }

  if (state === 'lobby' || (!playing && !ended)) {
    return (
      <main className="min-h-[100svh] flex flex-col items-center justify-center gap-6 px-6 text-center">
        <h1 className="text-2xl font-serif text-parchment">{en ? 'Lateral Mystery' : '海龟汤'} · {props.room.name}</h1>
        <p className="text-parchment/60 max-w-md">{en ? 'The host gives you an eerie “setup”; you close in on the truth by asking yes/no questions, and reveal when you’ve got it. Invite friends to ask along.' : '主持人会给出一个诡异的「汤面」，你们靠提是非题逼近真相，想到了就揭晓。可以叫朋友一起来问。'}</p>
        <div className="flex flex-col items-center gap-3 w-full max-w-md">
          <span className="text-sm text-parchment/50">{en ? 'Play with others — send them the link:' : '想多人一起玩，把链接发给同伴：'}</span>
        ...
        <button onClick={start} disabled={busy || generating}
          ...
          {busy || generating ? (en ? 'Crafting…' : '正在出题…') : (en ? 'Start · serve a mystery' : '开始 · 来一道海龟汤')}
        </button>
      </main>
    );
  }
  ...
  return (
    <main className="h-[100svh] flex flex-col overflow-hidden">
      <header className="px-4 py-3 border-b border-eldritch/20 flex items-center justify-between">
        <span className="font-serif text-parchment">{en ? 'Lateral Mystery' : '海龟汤'} · {props.room.name}</span>
        <span className="text-xs text-parchment/50">{ended ? (en ? 'Ended' : '已结束') : (en ? 'Ask yes/no → I answer Yes/No/Irrelevant/Yes and no' : '提是非题 → 我答 是/不是/无关/是也不是')}</span>
      </header>

      <div className="flex-1 min-h-0 overflow-y-auto px-4 py-4 space-y-3 max-w-3xl w-full mx-auto">
        {messages.map((m) => <SoupMsg key={m.id} m={m} mine={m.sender_player_id === props.myPlayerId} who={nameOf(m.sender_player_id)} en={en} />)}
        {busy && <div className="text-center text-parchment/40 italic text-sm">{en ? 'The host is thinking…' : '主持人思考中……'}</div>}
        <div ref={bottomRef} />
      </div>
      ...
      {!ended ? (
        <div className="border-t border-eldritch/20 px-4 py-3 space-y-2 max-w-3xl w-full mx-auto" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
          {guessing ? (
            <div className="space-y-2">
              <textarea value={guess} onChange={(e) => setGuess(e.target.value)} placeholder={en ? 'State the full truth as you see it…' : '说出你认为的完整真相（汤底）…'}
              ...
              <div className="flex gap-2">
                <button onClick={submitGuess} disabled={busy} className="flex-1 px-4 py-2 rounded bg-blood/80 hover:bg-blood text-parchment disabled:opacity-50">{en ? 'Submit answer' : '提交答案'}</button>
                <button onClick={() => setGuessing(false)} className="px-4 py-2 rounded bg-fog border border-parchment/30 text-parchment/70">{en ? 'Back to asking' : '返回提问'}</button>
              </div>
            </div>
          ) : (
            <>
              <div className="flex gap-2">
                <input value={text} onChange={(e) => setText(e.target.value)} onKeyDown={(e) => e.key === 'Enter' && ask()}
                  placeholder={en ? 'Ask a yes/no question…' : '提一个是非题…'} disabled={busy}
                ...
                <button onClick={ask} disabled={busy} className="px-5 py-2 rounded bg-eldritch/60 hover:bg-eldritch text-parchment disabled:opacity-50 shrink-0">{en ? 'Ask' : '提问'}</button>
              </div>
              <div className="flex gap-2">
                <button onClick={() => setGuessing(true)} className="flex-1 px-4 py-2 rounded bg-blood/70 hover:bg-blood text-parchment text-sm">{en ? 'Reveal answer' : '揭晓答案'}</button>
                <button onClick={reveal} className="px-4 py-2 rounded bg-fog border border-parchment/30 text-parchment/60 text-sm">{en ? 'See answer (give up)' : '看汤底（放弃）'}</button>
              </div>
            </>
          )}
        </div>
        ...
        <div className="border-t border-blood/40 px-4 py-3 text-center max-w-3xl w-full mx-auto" style={{ paddingBottom: 'max(0.75rem, env(safe-area-inset-bottom))' }}>
          <span className="text-parchment/70 text-sm">{en ? 'This round is over. Another one? Go home and create a new mystery room.' : '这局结束了。想再来一道？回首页创建新的海龟汤房间。'}</span>
        </div>
      )}
    </main>
  );
...
function SoupMsg({ m, mine, who, en }: { m: any; mine: boolean; who: string; en: boolean }) {
  ...
  if (m.sender_type === 'kp') {
    return <div className="text-center"><span className="inline-block px-3 py-1.5 rounded-full bg-fog border border-eldritch/40 text-parchment/90 text-sm">{en ? 'Host: ' : '主持人：'}{m.content}</span></div>;
  }
  ...
  return (
    <div className="flex flex-col items-end">
      <span className="text-xs text-parchment/40 mb-1">{who}{mine ? (en ? ' (you)' : '（你）') : ''}</span>
