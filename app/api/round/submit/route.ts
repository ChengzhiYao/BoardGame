...
export async function POST(req: Request) {
  ...
  const aReady = !!fresh?.player_a_ready;
  const bReady = !!fresh?.player_b_ready;
...
async function resolveRound(admin: any, roomId: string) {
  ...
  const ctxChars = (['A', 'B'] as const).map((seat) => {
    ...
    return { seat, name: c?.name || '调查员', occupation: c?.occupation || '', hp: `${c?.hp_current ?? '?'}/${c?.hp_max ?? '?'}`, san: `${c?.san_current ?? '?'}/${c?.san_max ?? '?'}`, skills: c?.skills ? Object.entries(c.skills).map(([k, v]: any) => `${k} ${v.total}`).join('、') : '' };
  });
...
下一步可行动作放进 guidance.options。骰子已判定，不要再请求。`;
  ...
  if (out.progress?.ending_triggered) {
    ...
    await admin.from('messages').insert({ room_id: roomId, sender_type: 'system', turn_no: round, content: '【结局】' + (out.progress.ending_text || '调查到此结束。'), payload: { type: 'ending' } });
  }
