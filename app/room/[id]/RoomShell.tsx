    ...
    tables.forEach((table) => {
      ch.on(
        'postgres_changes',
        {
          ...
          schema: 'public',
          table,
          filter: `${table === 'rooms' ? 'id' : 'room_id'}=eq.${props.room.id}`,
        },
        debouncedRefresh
      );
    });
      ...
      {playing ? (
        ...
        <main className="min-h-screen flex flex-col">
          ...
          <div className="flex-1 flex items-stretch justify-center px-4 py-6">
            <FlowView state={state} props={props} />
          </div>
        </main>
      )}
    </>
...
function FlowView({ state, props }: { state: string; props: ShellProps }) {
  switch (state) {
    case 'lobby':
      return <Lobby {...props} />;
    ...
    case 'case_locking':
      return (
        <Centered
          title="正在编织真相……"
          desc="守秘人正在后台构建这桩案件的完整隐藏档案：真相、幕后黑手、时间线与误导线索。这一切将被锁定，永不向你展示。"
          spinner
        />
      );
    case 'character_creation':
      return (
        <div className="w-full max-w-2xl flex flex-col gap-4">
          {props.caseQuality && <QualityCard q={props.caseQuality} />}
          <CharacterFlow {...props} />
        </div>
      );
    ...
    default:
      return <Centered title="未知状态" desc={state} />;
  }
}
  ...
  return (
    <div className="flex flex-col items-center justify-center gap-6 text-center max-w-md">
      ...
      <p className="text-parchment/60">
        {full ? '两位调查员均已到齐。' : '等待第二位调查员加入……'}（{props.initialPlayers.length}/2）
      </p>
      ...
      {full ? (
        ...
        <div className="flex flex-col items-center gap-3 w-full">
          <span className="text-sm text-parchment/50">把这条邀请链接发给你的同伴：</span>
          <code className="text-xs px-3 py-2 rounded bg-fog border border-eldritch/30 text-eldritch break-all w-full">
            {url}
          </code>
          <button
            onClick={() => {
  ...
  return (
    <div className="rounded-lg bg-fog border border-eldritch/30 p-4">
      <div className="flex items-center justify-between mb-3">
        <span className="text-sm text-parchment/70">本案质量评估</span>
        <span className={`text-2xl font-serif ${color}`}>{q.complexity}<span className="text-xs text-parchment/40"> /100</span></span>
      </div>
      <div className="flex flex-wrap justify-between gap-y-2">
        <Metric label="谜团层数" value={q.mystery_layers} />
        <Metric label="误导线索" value={q.red_herrings} />
        <Metric label="嫌疑人" value={q.suspects} />
        <Metric label="隐藏结局" value={q.hidden_endings} />
        <Metric label="预计时长" value={q.est_duration || '—'} />
      </div>
    </div>
  );
}
