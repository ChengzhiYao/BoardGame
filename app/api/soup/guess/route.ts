...
export async function POST(req: Request) {
  ...
  try {
    const { data, usage } = await callLLMJson<any>({
      system: buildSoupJudgePrompt(puzzle.surface, bot?.bottom || ''),
    ...
    await admin.from('messages').insert({ room_id: roomId, sender_type: 'kp', turn_no: 0, content: (data.solved ? '✅ 答对了！' : '❌ 还差一点。') + (data.comment ? ' ' + data.comment : ''), payload: { type: 'soup_answer' } });
    ...
    if (data.solved) {
      ...
      await admin.from('messages').insert({ room_id: roomId, sender_type: 'system', turn_no: 0, content: '【汤底揭晓】' + (bot?.bottom || ''), payload: { type: 'soup_reveal' } });
    }
