...
export default function Home() {
  ...
  const [gameMode, setGameMode] = useState<'coc' | 'soup' | 'td' | 'jbs' | 'botc'>('coc');
  ...
  function roomName() {
    const suffix = lang === 'en'
      ? (gameMode === 'soup' ? '’s Lateral Mystery' : gameMode === 'td' ? '’s Truth or Dare' : gameMode === 'jbs' ? '’s Murder Mystery' : gameMode === 'botc' ? '’s Bloodbound' : '’s Investigation')
      : (gameMode === 'soup' ? ' 的海龟汤' : gameMode === 'td' ? ' 的真心话大冒险' : gameMode === 'jbs' ? ' 的剧本杀' : gameMode === 'botc' ? ' 的血染' : ' 的调查');
    ...
    <main className="min-h-screen flex flex-col items-center justify-center gap-7 px-6 text-center">
      ...
      {mode === 'idle' && (
        <div className="flex gap-2 flex-wrap justify-center">
          {(['coc', 'soup', 'td', 'jbs', 'botc'] as const).map((k) => (
