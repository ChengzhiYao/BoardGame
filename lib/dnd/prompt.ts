...
// 冒险审查评分（接近出版模组水准）
export const DND_QUALITY_PASS = 70;
export function buildDndReviewSystem() {
  return `你是资深龙与地下城审稿人。下面是一份隐藏冒险蓝图。严格、挑剔地评估它作为一场单元跑团的质量，目标接近人工出版模组。
逐条检查：1.核心目标与反派动机是否清晰且有张力；2.节拍是否推进有起伏、不拖沓不卡关；3.关键 NPC 是否立体（秘密+诉求+互相冲突，有人说谎）；4.计划遭遇是否难度曲线合理、配置贴合队伍；5.是否有出人意料但合理的反转；6.高潮/Boss 战是否精彩、有两难或抉择；7.奖励是否相称。
只输出 JSON：{ "complexity": 0到100总体质量分（平庸套路低分，有巧思有张力高分）, "depth": 节拍/层数(2-5), "pass": true或false, "issues": ["具体问题，简短"] }`;
}
export function composeDndQuality(review: any, bp: any, length?: string) {
  const clamp = (n: number) => Math.max(0, Math.min(100, Math.round(Number(n) || 0)));
  return {
    complexity: clamp(review?.complexity),
    depth: Number(review?.depth) || (bp?.acts?.length || 3),
    encounters: (bp?.encounters || []).length,
    npcs: (bp?.npcs || []).length,
    twist: bp?.twist ? 1 : 0,
    est_duration: length || '',
    pass: !!review?.pass,
